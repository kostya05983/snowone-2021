...
import ru.kontur.kinfra.queue.OperationResult.Companion.fail
import ru.kontur.kinfra.queue.OperationResult.Companion.success
...
sealed class OperationResult<out V, out E> {
    ...
    companion object {
        fun <V> success(value: V): OperationResult<V, Nothing> = Success(value)

        fun <E> fail(value: E): OperationResult<Nothing, E> = Fail(value)
    }
}
...
inline fun <V, E, R> OperationResult<V, E>.fold(
    onSuccess: (V) -> R,
    onFailure: (E) -> R
): R = when (this) {
    is OperationResult.Success -> onSuccess(result)
    is OperationResult.Fail -> onFailure(error)
}

inline fun <E, R, V : R> OperationResult<V, E>.recover(transform: (exception: E) -> R): OperationResult<R, E> =
    when (this) {
        is OperationResult.Success -> this
        is OperationResult.Fail -> success(transform(error))
    }
